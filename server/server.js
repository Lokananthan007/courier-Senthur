...
const headerNames = {
  waybill_no: ['Waybill Number', 'AWB No', 'CONNOTE NO'],
  order_id: ['Order #', 'From', 'ORDER ID'],
  destination_city: ['city', 'Curr. Dest. HUB', 'DEST'],
  order_date: ['Order Date', 'Date', 'DATE'],
  weight: ['Weight', 'Wgt', 'W-GMS']
};
...
app.post('/api/saveData', (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query('SELECT Cannote_No FROM invoice_items WHERE Cannote_No IN (?)', [cannoteNos], (err, results) => {
      if (err) {
        return db.rollback(() => {
      ...
      db.query('INSERT INTO invoice_items (Cannote_No, Date, Destination, Weight, RS_PS, client_id) VALUES ?', [insertRows], (err) => {
        if (err) {
          return db.rollback(() => {
